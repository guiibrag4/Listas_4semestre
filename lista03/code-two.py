...
class Impressora:
    ...
    def imprimir(self, documento):
        with self.mutex:
            ...
            print(f'{self.nome} iniciou a impressão do documento: {documento}')
            time.sleep(self.tempo_impressao)
            print(f'{self.nome} conclui a impressão do documento: {documento}\n')
            self.imprimindo = False
            self.mutex.notify_all()
            
# Funçãao para simular a impressão
...
impressora2 = Impressora("Impressora 2", 5)

#Criando as threads paara os documentos
